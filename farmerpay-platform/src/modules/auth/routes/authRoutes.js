 ...
 */
// Refresh endpoint is rate-limited to defeat stolen-token spamming. A real
// client never refreshes more than once per few minutes; an attacker with a
// leaked refresh token would mint access tokens in a tight loop.
const refreshLimiter = createLimiter({ windowMs: 60 * 1000, max: isDev ? 100 : 5 });
 ...
 */
// /auth/me returns role + permissions. Cap it at 1 req/sec/user so an
// authenticated client can't spam it to map out the permission surface.
const meLimiter = createLimiter({ windowMs: 60 * 1000, max: isDev ? 1000 : 60 });
router.get('/me', authenticate, meLimiter, authController.getMe);
