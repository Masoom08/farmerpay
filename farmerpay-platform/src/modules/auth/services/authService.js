...
const generateAccessToken = (user, roleName) => {
  // Token payload holds only the opaque user UUID and role. Email and mobile
  // used to be embedded here; they leaked to anywhere the token surfaced
  // (proxy logs, error reports, browser devtools). Clients needing profile
  // fields should call /auth/me.
  return jwt.sign(
...
const register = async (data) => {
  ...
  if (existingUser) {
    // Avoid leaking account existence to callers. Return a decoy response that
    // is indistinguishable from a successful registration; no OTP is sent and
    // nothing is persisted. A real user repeating registration will not get an
    // OTP — they'll find out via the login flow (which legitimately reveals
    // existence only after MPIN-backed authentication). Scraping for known
    // users now costs an attacker the rate-limit budget without yielding a
    // signal.
    logger.warn(
...
const sendOtp = async (data) => {
  ...
  // Rate check: max 3 OTPs per identifier per 10 minutes (burst) AND max 10
  // per identifier per 24 hours (daily cap). The 24h cap defeats the
  // rotating-proxy SMS-flood attack: even if an attacker defeats the
  // 10-minute window by waiting, they cannot exhaust the victim's daily
  // SMS quota.
  const identifierWhere = formattedMobile ? { mobile: formattedMobile } : { email: email.toLowerCase() };
  const tenMinutesAgo = new Date(Date.now() - 10 * 60 * 1000);
  const oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);

  const [recentCount, dailyCount] = await Promise.all([
    OtpRequest.count({
      where: { ...identifierWhere, purpose, created_at: { [Op.gte]: tenMinutesAgo } },
    }),
    OtpRequest.count({
      where: { ...identifierWhere, created_at: { [Op.gte]: oneDayAgo } },
    }),
  ]);

  const otpMaxAttempts = process.env.NODE_ENV !== 'production' ? 100 : 3;
  const otpDailyCap = process.env.NODE_ENV !== 'production' ? 1000 : 10;
  if (recentCount >= otpMaxAttempts || dailyCount >= otpDailyCap) {
...
const verifyOtp = async (data) => {
  ...
  if (inputBuffer.length !== storedBuffer.length || !crypto.timingSafeEqual(inputBuffer, storedBuffer)) {
    // Don't leak the remaining-attempt count; attackers use it to tune
    // their brute-force rate (AUTH-H9).
...
const setMpin = async (data) => {
  ...
  // Revoke every active session on MPIN (re)set. setMpin is called for both
  // initial registration and forgot-mpin flows; in the latter, any session
  // belonging to a prior holder of the account must be force-logged-out.
...
const login = async (data, meta = {}) => {
  ...
  // Check account lockout. We never auto-unlock after the timeout expires —
  // once a user crosses LOGIN_MAX_FAILURES their session is suspended until
  // they complete a forgot-mpin flow (OTP-backed reset). Auto-unlock made
  // this endpoint trivially brute-forceable: wait LOGIN_LOCKOUT_MINUTES and
  // try again.
  if (user.failed_login_attempts >= LOGIN_MAX_FAILURES) {
    const err = new Error('Account locked after repeated failures. Reset via forgot-mpin');
    err.statusCode = 423;
    err.errorCode = 'AUTH_006';
    throw err;
  }
  ...
  if (!isMatch) {
    ...
    // Don't leak attempt count to the caller (AUTH-H9). Server-side audit
    // trail in logger.warn above is sufficient for investigation.
    const err = new Error('Invalid credentials');
    err.statusCode = 401;
    err.errorCode = 'AUTH_002';
    throw err;
...
const logout = async (userUuid, accessToken) => {
  ...
  if (session) {
    ...
    // Also revoke any active Aadhaar step-up. Without this the Tier-2
    // window (default 15 min) outlives the session, letting a stolen
    // device resume privileged DICE actions for minutes after logout.
    try {
      const aadhaarAuthService = require('./aadhaarAuthService');
      if (aadhaarAuthService && typeof aadhaarAuthService.revokeStepUp === 'function') {
        await aadhaarAuthService.revokeStepUp(session.user_id);
      }
    } catch (err) {
      logger.warn(`[logout] Aadhaar step-up revoke failed for user ${userUuid}: ${err.message}`);
    }
  }
