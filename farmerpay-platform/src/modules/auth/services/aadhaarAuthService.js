...
const sendOtpToUidai = async (aadhaar) => {
  ...
  logger.info(`[MOCK UIDAI] OTP sent for Aadhaar ending ${aadhaar.slice(-4)} (txn: ${txnId})`);
  return { otp, txnId };
};
...
const initiateAadhaarOtp = async (userId, aadhaar, meta = {}) => {
  ...
  // Aadhaar is a high-value identifier under DPDP/UIDAI rules. Every touch
  // must be reconstructable from an audit trail independent of the mutable
  // AadhaarVerification row. Only the last 4 digits + hash leave this
  // function; the raw 12-digit number exists only in the request memory.
  logger.info('aadhaar.otp_initiate', {
    verificationUuid: record.verification_uuid,
    userId,
    aadhaarLast4: record.aadhaar_last4,
    ip: meta.ipAddress || null,
    device: meta.deviceFingerprint || null,
  });
