...
const addAnimal = async (farmerId, data) => {
  ...
  const animal = await DairyAnimal.create({
    ...
    current_lifecycle_stage: data.lifecycleStage || 'HEIFER', // see VALID_LIFECYCLE
    status: 'ACTIVE',
    primary_photo_url: data.primaryPhotoUrl || null,
    notes: data.notes || null,
    animal_type: LEGACY_ANIMAL_TYPE_MAP[data.species || 'CATTLE'] || 'cow', // legacy enum is lowercase
  });
