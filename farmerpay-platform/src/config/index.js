...
const config = {
  ...
  appName: process.env.APP_NAME || 'FarmerPay',
  ...
  db: {
    ...
    password: process.env.DB_PASSWORD || '#Masoom08',
    ...
    readPort: parseInt(process.env.DB_READ_PORT, 10) || 3306,
  ...
  redis: {
    ...
    keyPrefix: process.env.REDIS_KEY_PREFIX || 'farmerpay:',
  },
  ...
  accountAggregator: {
    ...
    setuClientId: process.env.AA_SETU_CLIENT_ID || null,
    setuClientSecret: process.env.AA_SETU_CLIENT_SECRET || null,
    finvuApiKey: process.env.AA_FINVU_API_KEY || null,
  },
  ...
  features: {
    ...
    // Readiness — per-persona feature gates. Default OFF in production.
    // Set to 'true' in staging .env to enable progressively.
    readiness: {
      farmerBadge: process.env.FEATURE_READINESS_FARMER_BADGE === 'true',
      sathiCoachingPriority: process.env.FEATURE_READINESS_SATHI_COACHING === 'true',
      bankerMatrix: process.env.FEATURE_READINESS_BANKER_MATRIX === 'true',
      sathiShadowLog: process.env.FEATURE_READINESS_SATHI_SHADOW_LOG === 'true',
    },
