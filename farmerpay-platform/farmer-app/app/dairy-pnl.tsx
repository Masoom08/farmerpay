  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [h, p] = await Promise.all([
        apiGet(`/roots/dairy/v2/pnl/herd?startDate=${start}&endDate=${end}`),
        apiGet(`/roots/dairy/v2/pnl/per-animal?startDate=${start}&endDate=${end}`),
      ]);
    ...
    <ScrollView style={s.scroll} contentContainerStyle={s.content}>
      ...
      <View style={[s.chipRow, { marginTop: 10 }]}>
        <TouchableOpacity style={[s.toggleBtn, view === "herd" && s.toggleBtnSel]} onPress={() => setView("herd")}>
          <Text style={[s.toggleText, view === "herd" && s.toggleTextSel]}>Herd Total</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[s.toggleBtn, view === "animal" && s.toggleBtnSel]} onPress={() => setView("animal")}>
          <Text style={[s.toggleText, view === "animal" && s.toggleTextSel]}>Per-Animal</Text>
        </TouchableOpacity>
      </View>
      ...
      {view === "herd" ? (
        ...
        <View>
          {perAnimal.length === 0 ? (
            <View style={s.card}>
              <Text style={s.empty}>No animals in herd</Text>
            </View>
          ) : (
            perAnimal.map((a) => {
              ...
              return (
                <View key={a.animalUuid} style={s.animalCard}>
                  <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 8 }}>
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={s.animalName}>{a.name || a.tagNumber || "Unnamed"}</Text>
