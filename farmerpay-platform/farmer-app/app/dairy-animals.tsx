...
const LIFECYCLES: { value: Lifecycle; label: string }[] = [
  ...
  { value: "DRY", label: "Dry" },
  { value: "EARLY_LACTATION", label: "Early Lact." },
  { value: "PEAK_LACTATION", label: "Peak Lact." },
  { value: "LATE_LACTATION", label: "Late Lact." },
...
export default function DairyAnimals() {
  ...
  const resetForm = () => {
    setTag(""); setName(""); setSpecies("CATTLE"); setBreed("");
  ...
  const save = async () => {
    if (!name.trim() && !tag.trim()) {
      Alert.alert("Missing", "Enter at least a name or tag number");
      return;
    }
    setSaving(true);
    try {
      ...
      const res = await apiPost("/roots/dairy/v2/animals", body);
      if (res.success) {
        Alert.alert("Added", "Animal added to herd");
        resetForm();
        setShowForm(false);
        load();
      } else {
        Alert.alert("Error", res.message || "Failed to add animal");
      }
    } catch (e: any) {
  ...
  return (
    <ScrollView style={s.scroll} contentContainerStyle={s.content}>
      <View style={s.header}>
        ...
        <Text style={s.headerTitle}>My Herd / मेरे पशु</Text>
      </View>

      {!showForm && (
        <TouchableOpacity style={s.addBtn} onPress={() => setShowForm(true)} activeOpacity={0.8}>
          <Text style={s.addBtnText}>+ Add Animal / पशु जोड़ें</Text>
        </TouchableOpacity>
      )}

      {showForm && (
        <View style={s.card}>
          <Text style={s.cardLabel}>New Animal / नया पशु</Text>

          <Text style={s.fieldLabel}>Tag Number</Text>
          ...
          <Text style={s.fieldLabel}>Species</Text>
          <View style={s.chipRow}>
            {(["CATTLE", "BUFFALO"] as Species[]).map((v) => (
              <TouchableOpacity key={v} style={[s.chip, species === v && s.chipSel]} onPress={() => setSpecies(v)}>
                <Text style={[s.chipText, species === v && s.chipTextSel]}>{v === "CATTLE" ? "🐄 Cattle" : "🐃 Buffalo"}</Text>
              </TouchableOpacity>
            ))}
          </View>

          <Text style={s.fieldLabel}>Breed Code</Text>
          ...
          <View style={s.chipRow}>
            {(["FEMALE", "MALE"] as Gender[]).map((v) => (
              <TouchableOpacity key={v} style={[s.chip, gender === v && s.chipSel]} onPress={() => setGender(v)}>
                <Text style={[s.chipText, gender === v && s.chipTextSel]}>{v}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          <Text style={s.fieldLabel}>Lifecycle Stage</Text>
          <View style={s.chipRow}>
            {LIFECYCLES.map((l) => (
              <TouchableOpacity key={l.value} style={[s.chipSmall, lifecycle === l.value && s.chipSel]} onPress={() => setLifecycle(l.value)}>
                <Text style={[s.chipTextSmall, lifecycle === l.value && s.chipTextSel]}>{l.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          <View style={{ flexDirection: "row", gap: 10, marginTop: 16 }}>
            ...
            <TouchableOpacity style={[s.saveBtn, saving && { opacity: 0.6 }]} disabled={saving} onPress={save}>
              {saving ? <ActivityIndicator color="#fff" /> : <Text style={s.saveBtnText}>Save Animal</Text>}
            </TouchableOpacity>
          </View>
        </View>
      )}
      ...
      {animals.length === 0 && !showForm ? (
        <View style={s.empty}>
          ...
          <Text style={s.emptySub}>अभी कोई पशु नहीं</Text>
        </View>
      ) : (
        animals.map((a) => (
          <View key={a.animal_uuid} style={s.animalCard}>
            <Text style={s.animalEmoji}>{a.species === "BUFFALO" ? "🐃" : "🐄"}</Text>
            <View style={{ flex: 1 }}>
              <Text style={s.animalName}>{a.name || a.tag_number || "Unnamed"}</Text>
              <Text style={s.animalMeta}>
                {a.tag_number ? `#${a.tag_number} · ` : ""}
                {a.breed_code || a.species} · {a.current_lifecycle_stage?.replace(/_/g, " ")}
              </Text>
              {a.purchase_cost && (
                <Text style={s.animalCost}>Bought: {formatRupees(a.purchase_cost)}</Text>
              )}
            </View>
          </View>
        ))
