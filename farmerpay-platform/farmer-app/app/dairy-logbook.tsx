...
export default function DairyLogbook() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const pRes = await apiGet("/roots/dairy/v2/profile");
      if (pRes.success) setProfile(pRes.data);

      const aRes = await apiGet("/roots/dairy/v2/animals");
      if (aRes.success) setAnimals(aRes.data || []);

      const start = daysAgo(14);
      const end = daysAgo(0);
      const pnlRes = await apiGet(`/roots/dairy/v2/pnl/herd?startDate=${start}&endDate=${end}`);
  ...
  if (!profile) {
    return (
      <View style={s.center}>
        ...
        <Text style={s.emptyTitle}>Set up your dairy first</Text>
        <Text style={s.emptySub}>पहले अपनी डेयरी सेट करें</Text>
  ...
  const actions = [
    { icon: "🥛", label: "Log Milk Sale",   labelHi: "दूध बिक्री",   route: "/dairy-log-revenue" },
    { icon: "💰", label: "Log Expense",     labelHi: "खर्च",        route: "/dairy-log-cost" },
    { icon: "🐄", label: "My Animals",      labelHi: "मेरे पशु",     route: "/dairy-animals" },
    { icon: "💉", label: "Vet Treatment",   labelHi: "पशु चिकित्सा",  route: "/dairy-treatment" },
    { icon: "🤰", label: "Breeding",        labelHi: "प्रजनन",       route: "/dairy-breeding" },
    { icon: "📊", label: "P&L Report",      labelHi: "लाभ-हानि",     route: "/dairy-pnl" },
  ];
  ...
  return (
    <ScrollView style={s.scroll} contentContainerStyle={s.content}>
      ...
      <View style={s.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={s.headerTitle}>Dairy Logbook</Text>
          <Text style={s.headerSub}>
            {profile.herd_tier} tier · {profile.entry_mode === "WEEKLY_BULK" ? "Weekly" : "Daily"} entry
          </Text>
        </View>
      ...
      <TouchableOpacity style={s.card} onPress={() => router.push("/dairy-animals" as any)} activeOpacity={0.7}>
        <Text style={s.cardLabel}>My Herd / मेरे पशु</Text>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
          <Text style={s.herdCount}>{animals.length}</Text>
          <View style={{ flex: 1 }}>
            <Text style={s.herdLabel}>animals active</Text>
            <Text style={s.herdSub}>
              {animals.filter((a) => a.current_lifecycle_stage?.includes("LACTATION")).length} lactating ·{" "}
              {animals.filter((a) => a.current_lifecycle_stage === "PREGNANT").length} pregnant
            </Text>
          </View>
