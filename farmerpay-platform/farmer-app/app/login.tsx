...
import {
  ...
  ActivityIndicator, KeyboardAvoidingView, Platform, Alert, AlertButton,
} from "react-native";
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const ok = await authenticateWithBiometrics("Unlock FarmerPay");
    ...
    try {
      ...
      if (res.success && res.data?.accessToken) {
        ...
        if (bio.hardwareAvailable && bio.enrolled && !bio.enabled) {
          showAlert(
            "Faster sign-in?",
            `Use ${bio.capability === "FACE" ? "Face ID" : "fingerprint"} to unlock FarmerPay next time.`,
            [
      ...
      if (e?.message === "UNAUTHORIZED") {
        showAlert("Login Failed", "Invalid mobile number or MPIN. Please try again.");
      } else {
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={styles.card}>
        ...
        <Text style={styles.title}>FarmerPay</Text>
        ...
        <Text style={styles.footer}>Secured by FarmerPay Platform</Text>
      </View>
